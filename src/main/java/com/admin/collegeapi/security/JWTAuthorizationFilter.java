...
public class JWTAuthorizationFilter extends BasicAuthenticationFilter {
    ...
    private UsernamePasswordAuthenticationToken getAuthentication(HttpServletRequest request) {
        ...
        if (token != null) {
            // parse the token.
            String extractedToken = JWT.require(Algorithm.HMAC512(SECRET.getBytes()))
                    .build()
                    .verify(token.replace(TOKEN_PREFIX, ""))
                    .getSubject();

            List<GrantedAuthority> authorities = new ArrayList<>();
            authorities.add(new SimpleGrantedAuthority("ROLE_"+extractedToken.split(":")[2]));
            if (extractedToken.split(":")[0] != null) {
                return new UsernamePasswordAuthenticationToken(extractedToken.split(":")[0], null, authorities);
            }
        }
        return null;
    }
